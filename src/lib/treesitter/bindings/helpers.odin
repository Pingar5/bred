...
compat_allocator_proc :: proc(allocator_data: rawptr, mode: mem.Allocator_Mode,
	...
	case .Free:
		...
		free_old_size   := int(header.size)
		old_memory := header

		return rra.parent.procedure(rra.parent.data, mode, size, alignment, old_memory, free_old_size, location)

	case .Resize:
		header := cast(^Header)(uintptr(old_memory)-size_of(Header))

		resize_size       := size + size_of(Header)
		resize_old_size   := int(header.size)
		old_memory := header

		data = rra.parent.procedure(rra.parent.data, mode, resize_size, alignment, old_memory, resize_old_size, location) or_return
